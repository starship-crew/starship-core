...
def make_combat(sess, crew1, crew2):
...
def combat_connector():
    ...
    with db.session() as sess:
        while True:
            for crew1 in sess.query(Crew).filter(Crew.searching):
                found = False
                for crew2 in sess.query(Crew).filter(
                    Crew.searching,
                    Crew.id != crew1.id,
...
def apply_action(sess, crew, enemy):
...
def combat_action_handler():
    ...
    with db.session() as sess:
        while True:
            for crew in sess.query(Crew).filter(Crew.active, Crew.action != None):
                enemy = crew.opponent
                crew.active = False
                enemy.active = True
                apply_action(sess, crew, enemy)
                crew.action = None
                sess.commit()
            sleep(0.5)
