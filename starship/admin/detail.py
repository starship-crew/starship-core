...
def change_detail_type_order(id):
    with db.session() as db_sess:
        ...
        if dir == "up":
            if not (
                dt_prev := db_sess.query(DetailType)
                .filter_by(order=dt.order - 1)
                .first()
            ):
                return redirect(redirect_url())
            dt.order -= 1
            ...
            if not (
                dt_next := db_sess.query(DetailType)
                .filter_by(order=dt.order + 1)
                .first()
            ):
                return redirect(redirect_url())
...
def create_detail():
    with db.session() as db_sess:
        ...
        form.kind.choices = [
            (dt.id, dt.name.get(lang)) for dt in get_detail_types(db_sess)
        ]
...
def change_detail_copy_level(id, value):
    with db.session() as db_sess:
        ...
        try:
            ...
            flash(
                f'Failed to change level to "{value}" cause of data processing issues'
            )
